...
export default function UrlDecode() {
  ...
  const decode = () => {
    try { setOutput(decodeURIComponent(input)); } catch { setOutput('解码失败'); }
  };
  const copyOutput = () => { navigator.clipboard.writeText(output); setCopied(true); setTimeout(() => setCopied(false), 2000); };

  return (
    <div className="tool-page-container">
      <div className="tool-header"><h1 className="tool-header-title">URL解码</h1><p className="tool-header-desc">解码URL编码数据</p></div>
      ...
      <div className="tool-io-grid">
        <div className="tool-input-section"><label className="tool-label">URL编码输入</label><textarea className="tool-textarea" value={input} onChange={(e) => setInput(e.target.value)} placeholder="输入URL编码..." /></div>
